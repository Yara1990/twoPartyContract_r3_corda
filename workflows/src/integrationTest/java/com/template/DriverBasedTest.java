...
public class DriverBasedTest {
    ...
    public void nodeTest() {
        driver(new DriverParameters().withIsDebug(true).withStartNodesInProcess(true), dsl -> {
            ...
            try {
                NodeHandle hsbcHandle = handleFutures.get(0).get();
                NodeHandle wellsFargoHandle = handleFutures.get(1).get();
                ...
                assertEquals(hsbcHandle.getRpc().wellKnownPartyFromX500Name(bankB.getName()).getName(), bankB.getName());
                assertEquals(wellsFargoHandle.getRpc().wellKnownPartyFromX500Name(bankA.getName()).getName(), bankA.getName());
            } catch (Exception e) {
